...
export default function ReviewApplicationsPage() {
  ...
  const act = async (id: string, status: 'approved' | 'rejected') => {
    setBusy(id);
    try {
      const updated = await adminApi.updateApplication(id, status);
    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-grow max-w-container-max mx-auto w-full px-margin-mobile md:px-margin-desktop py-stack-lg">
        ...
        <div className="space-y-stack-sm">
          {apps.map((a) => (
            <div key={a._id} className="bg-surface-container-lowest p-stack-md rounded-xl border border-outline-variant flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="font-body-sm text-body-sm text-on-surface-variant">
                    {a.user ? `@${a.user.username}` : 'unknown user'} · applied {fmtDate(a.createdAt)}
                  </p>
